...
- **Статус экспорта:** ⚠️ частично — GLB-анимации создаются только для Sette ObjectId 1/2;
  остальные запекаются в позицию кадра 0. Пример связки: Sette mg88–92 → ObjectId 9 (статичный,
  Frames=0, Y/Z-swap + сдвиг); attr=8192 группы требуют per-map политики (см. фикс 5b33f33).
...
|---|---|---|
| BSP anim objects (треки, parent) | ✅ | частично (Sette obj1/2) |
...
|---|---|---|
| Обобщить анимации BSP на все объекты (не только Sette 1/2) | 0.5–1 д | готовые сэмплеры |
| Иерархия нод по Parent для BSP-объектов | 0.5 д | аккуратно с MirrorY |
