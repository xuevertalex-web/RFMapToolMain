...
echo [smoke-gate] dotnet build LocalCursorAgent.csproj
dotnet build LocalCursorAgent.csproj
if errorlevel 1 (
  echo [smoke-gate] FAILED: build step failed.
  exit /b 1
)

echo [smoke-gate] dotnet run --project SafetyTests\SafetyTests.csproj
for /f "usebackq delims=" %%L in (`dotnet run --project SafetyTests\SafetyTests.csproj 2^>^&1`) do (
  echo %%L
