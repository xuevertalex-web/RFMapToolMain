...
$reportScript = Join-Path $scriptRoot "Report-SourceSnapshotContents.ps1"
...
& $reportScript -ArchivePath $latest.FullName
& $verifyScript -ArchivePath $latest.FullName

Write-Host "Snapshot create/report/verify completed: $($latest.FullName)"
