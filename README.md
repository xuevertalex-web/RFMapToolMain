...
## Неинтерактивные флаги (batch mode)

- `--all` — экспорт всех карт без интерактивного меню.
- `--map Sette` — экспорт по части имени карты.
- `--filter=Vol*` — экспорт по маске (`*` и `?`, без учёта регистра).
- `--resume` — пропускать карты, у которых GLB новее всех исходников.
- `--validate` — после экспорта перечитывает каждый GLB через SharpGLTF (round-trip),
  проверяет текстуры/материалы и выставляет exit code: `0`=OK, `1`=warnings, `2`=errors.

## Диагностика текстур

- Логи стадий `[TEX]` (загрузка `.r3t`), `[CONV]` (DDS→PNG с форматом DXT1/3/5/ATI/BC7),
  `[GLTF]` (привязка текстур к материалам).
- После каждой карты пишется `RF_Release/<map>/texture_report.json`
  (счётчики total/found/converted/embedded/failed и список материалов без текстур).
