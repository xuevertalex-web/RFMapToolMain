...
- **Анимации BSP-объектов работают на всех картах** (раньше — только Sette):
  24 карты с объектами получили иерархию `BSP_obj*` и TRS-каналы
  (напр. Elan — 35 объектов/601 кадр, lobby — 13/2001, Transport01 — 34/301).
  Для этого снят глобальный skip трансформа для групп `attr=8192`
  (все object-группы на всех картах имеют этот attr; skip оставлял объекты
  в raw local space — «в центре карты / за картой» — и глушил анимации).
